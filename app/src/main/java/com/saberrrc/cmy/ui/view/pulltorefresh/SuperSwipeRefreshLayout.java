...
/**
 * 自定义CustomeSwipeRefreshLayout<br>
 * 支持下拉刷新和上拉加载更多<br>
...
public class SuperSwipeRefreshLayout extends ViewGroup {
    ...
    private float   mInitialTouchMotionY;
    private float   mInitialMotionY;
    private boolean mIsBeingDragged;
    private int mPullToRefreshStatus = 1;//1-下拉，2-上拉，0-无动作
    ...
    private HeadViewContainer mFooterViewContainer;
    ...
    protected boolean getIsBeginDragged() {
    ...
    private void createFooterViewContainer() {
        mFooterViewContainer = new HeadViewContainer(getContext());
        mFooterViewContainer.setVisibility(View.GONE);
        addView(mFooterViewContainer);
    }

    public void setFooterViewContainerGone() {
        //        mFooterViewContainer.setVisibility(View.GONE);
        mLoadMore = false;
        pushDistance = 0;
        resetTargetLayout();
        mTarget.scrollBy(0, mTarget.getScrollY() + mFooterViewHeight);
    }
    ...
    private void setRefreshing(boolean refreshing, final boolean notify) {
        if (mRefreshing != refreshing) {
            ...
            if (mRefreshing) {
                ...
                startScaleDownReturnToStartAnimation(mCurrentTargetOffsetTop, mRefreshListener);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        mHeadViewContainer.layout((width / 2 - headViewWidth / 2), mCurrentTargetOffsetTop, (width / 2 + headViewWidth / 2), mCurrentTargetOffsetTop + headViewHeight);// 更新头布局的位置
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mHeadViewContainer.measure(MeasureSpec.makeMeasureSpec(mHeaderViewWidth, MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(3 * mHeaderViewHeight, MeasureSpec.EXACTLY));
        mFooterViewContainer.measure(MeasureSpec.makeMeasureSpec(mFooterViewWidth, MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(mFooterViewHeight, MeasureSpec.EXACTLY));
    ...
    private boolean measureTouch(MotionEvent ev, View childView) {
        try {
            float childL = childView.getX();
            float childT = childView.getY();
            float childR = childView.getWidth() + childL;
            float childB = childView.getHeight() + childT;
            float touchX = ev.getX();
            float touchY = ev.getY();
            if (touchX >= childL && touchX <= childR && touchY >= childT && touchY <= childB) {
                return true;
            }
            return false;
        } catch (Exception e) {
    ...
    public void setExceptionview(View eptionview) {
        this.eptionview = eptionview;
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        boolean isScrollToTop = isChildScrollToTop();
        boolean isScrollToBottom = isChildScrollToBottom();
        if (!isEnabled() || mReturningToStart || mRefreshing || mLoadMore || (!isScrollToTop && !isScrollToBottom)) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                lock = measureTouch(ev, eptionview);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isScrollToTop) {
                    ...
                    if (yDiff > mTouchSlop && !mIsBeingDragged) {// 判断是否下拉的距离足够
                        mIsBeingDragged = refreshEnable;// 正在下拉
                        mPullToRefreshStatus = 1;
                    } else if (isScrollToBottom && yDiff < 0 && Math.abs(yDiff) > mTouchSlop && !mIsBeingDragged) {
                        mIsBeingDragged = true;// 正在上拉
                        mPullToRefreshStatus = 2;
                    }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        boolean isScrollToTop = isChildScrollToTop();
        boolean isScrollToBottom = isChildScrollToBottom();
        //        MyLogger.getLogger().d("qqqqq onTouchEvent");
        boolean touch = measureTouch(ev, eptionview);
        if (!isEnabled() || mReturningToStart || (!isScrollToTop && !isScrollToBottom) || touch) {
        ...
        if (isScrollToTop && mPullToRefreshStatus == 1) {// 下拉刷新
            if (!refreshEnable) {
                return false;
            }
            return handlerPullTouchEvent(ev, action);
        } else if (isScrollToBottom && mPullToRefreshStatus == 2) {// 上拉加载更多
            if (!loadMoreEnable) {
                return false;
            }
            return handlerPushTouchEvent(ev, action);
    ...
    private boolean handlerPullTouchEvent(MotionEvent ev, int action) {
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (pointerIndex < 0) {
                    LogUtil.d("Got ACTION_MOVE event but have an invalid active pointer id.");
                    return false;
                }
            ...
            case MotionEvent.ACTION_CANCEL: {
                if (mActivePointerId == INVALID_POINTER) {
                    if (action == MotionEvent.ACTION_UP) {
                        LogUtil.d( "Got ACTION_UP event but don't have an active pointer id.");
                    }
                    return false;
                }
                final int pointerIndex = MotionEventCompat.findPointerIndex(ev, mActivePointerId);
    ...
    private boolean handlerPushTouchEvent(MotionEvent ev, int action) {
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                mActivePointerId = MotionEventCompat.getPointerId(ev, 0);
                mIsBeingDragged = false;
                LogUtil.d( "debug:onTouchEvent ACTION_DOWN");
                break;
            case MotionEvent.ACTION_MOVE: {
                final int pointerIndex = MotionEventCompat.findPointerIndex(ev, mActivePointerId);
                if (pointerIndex < 0) {
            ...
            case MotionEvent.ACTION_CANCEL: {
                ...
                final int pointerIndex = MotionEventCompat.findPointerIndex(ev, mActivePointerId);
                final float y = MotionEventCompat.getY(ev, pointerIndex);
                float overscrollBottom = (mInitialMotionY - y) * DRAG_RATE;// 松手是下拉的距离
                mIsBeingDragged = false;
                mActivePointerId = INVALID_POINTER;
                if (overscrollBottom < 0) {
                    pushDistance = 0;
                    overscrollBottom = 0;
                } else if (overscrollBottom < mFooterViewHeight || mOnPushLoadMoreListener == null) {// 直接取消
                    pushDistance = 0;
    ...
    private void animatorFooterToBottom(final int start, final int end) {
