...
public class LogUtil {
    ...
    public static class Builder {
        ...
        public Builder() {
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
                dir = App.getInstance().getExternalCacheDir() + File.separator + "log" + File.separator;
            } else {
                dir = App.getInstance().getCacheDir() + File.separator + "log" + File.separator;
            }
        }
    ...
    private static String[] processContents(int type, String tag, Object... contents) {
        ...
        String head = new Formatter().format("Thread: %s, %s(%s.java:%d)" + LINE_SEPARATOR, Thread.currentThread().getName(), targetElement.getMethodName(), className, targetElement.getLineNumber()).toString();
        ...
        if (contents != null) {
            if (contents.length == 1) {
                ...
                for (int i = 0, len = contents.length; i < len; ++i) {
                    ...
                    sb.append(ARGS).append("[").append(i).append("]").append(" = ").append(content == null ? NULL : content.toString()).append(LINE_SEPARATOR);
                }
                msg = sb.toString();
            }
        }
    ...
    private synchronized static void print2File(final String tag, final String msg) {
        ...
        sb.append(time).append(tag).append(": ").append(msg).append(LINE_SEPARATOR);
        if (sLogBorderSwitch)
